...
const ImageIcon = () => <div className="w-12 h-12 text-gray-400">Img</div>;
    ...
    <div className="fixed inset-0 bg-black/60 backdrop-blur-sm flex items-center justify-center z-50 p-4 overflow-y-auto">
      ...
      >
        <div className="sticky top-0 bg-white p-6 border-b z-10">
          <h2 className="text-xl font-bold text-slate-800">{title || "Modal"}</h2>
      ...
      <div className="flex flex-col lg:flex-row gap-6">
        <div className="lg:w-1/3">
          ...
          <div className="aspect-video w-full mb-5 bg-slate-100 rounded-lg flex items-center justify-center overflow-hidden">
            {photoPreview ? (
              <img src={photoPreview} alt="Property preview" className="w-full h-full object-cover" onError={() => setPhotoPreview(null)} />
            ) : (
              <ImageIcon />
            )}
          </div>
...
export function SellerProperties() {
  ...
  const fetchProperties = async () => {
    setLoading(true);
    try {
      const response = await fetch(`${API_URL}/api/seller/properties`, {
      ...
      toast.error('Error loading properties. Using mock data for testing.');
      console.error(err);
  ...
  const handleFormSubmit = async (formData) => {
    ...
    const endpoint = modalState.type === 'edit'
      ? `${API_URL}/api/seller/properties/${modalState.data?._id}`
      : `${API_URL}/api/seller/properties`;
  ...
  const handleDelete = async () => {
    try {
      const response = await fetch(`${API_URL}/api/seller/properties/${modalState.data?._id}`, {
        method: 'DELETE',
        headers: { 'Authorization': `Bearer ${localStorage.getItem('token')}` }
      });
  ...
  const FallbackIcon = () => <span>Icon</span>; 
    ...
    <>
      ...
      <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} className="p-6">
        <div className="flex flex-col md:flex-row justify-between items-center mb-6 gap-4">
          <h2 className="text-3xl font-bold text-slate-800">My Properties</h2>
          <div className="flex flex-col sm:flex-row items-center gap-3 w-full md:w-auto">
            ...
            <button
              ...
              className="w-full sm:w-auto flex items-center justify-center gap-2 px-4 py-2 bg-indigo-600 text-white rounded-lg shadow-md hover:bg-indigo-700"
            >
              {PlusCircle ? <PlusCircle size={20} /> : <FallbackIcon />}
              Add Property
            </button>
          </div>
        </div>
        ...
        {loading ? (
          ...
          <p className="text-center p-8 text-slate-500">No properties found. Add a new property to get started.</p>
        ) : (
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
            {filteredProperties.map(prop => (
              <motion.div
                key={prop._id || Math.random().toString()}
              ...
              >
                <div className="relative">
                  <img
                    ...
                    onError={(e) => e.target.src = "https://placehold.co/600x400/e2e8f0/475569?text=Error"}
                  />
                  <div className="absolute top-2 right-2 flex gap-2 opacity-0 group-hover:opacity-100 transition-opacity">
                    <button
                      onClick={() => openModal('edit', prop)}
                      className="bg-white/80 p-1.5 rounded-full text-indigo-600 hover:bg-white"
                    >
                      <Edit size={16} />
                    </button>
                    <button
