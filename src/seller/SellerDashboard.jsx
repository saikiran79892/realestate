...
const getPageTitle = (pathname) => {
  ...
  if (!routeName || routeName === 'seller') return 'Dashboard';
...
export function SellerDashboard() {
  ...
  const sidebarClasses = `fixed inset-y-0 left-0 z-40 bg-gradient-to-b from-indigo-900 to-purple-900 text-white flex flex-col transition-all duration-300 ease-in-out shadow-2xl`;
  ...
  const navItems = [
    ...
    { to: '/seller/properties', label: 'My Properties', icon: BuildingOffice2Icon },
    { to: '/seller/appointments', label: 'My Appointments', icon: CalendarIcon },
  ...
  return (
    <div className="bg-gray-50 min-h-screen">
        ...
        <Dialog as="div" className="relative z-50" onClose={() => setIsMobileMenuOpen(false)}>
          ...
          <div className="fixed inset-0 overflow-y-auto">
            <div className="flex min-h-full items-start justify-start text-center">
              ...
              >
                <Dialog.Panel className="w-3/4 max-w-xs h-screen transform bg-gradient-to-b from-indigo-900 to-purple-900 text-white p-6 shadow-xl transition-all">
                  <div className="flex justify-between items-center mb-8">
                    <div className="flex items-center gap-2">
                      <BuildingOffice2Icon className="h-8 w-8 text-blue-300" />
                      <span className="text-xl font-bold bg-gradient-to-r from-blue-300 to-indigo-300 bg-clip-text text-transparent">EstateFlow</span>
                    </div>
                    <button onClick={() => setIsMobileMenuOpen(false)} className="p-2 rounded-full hover:bg-indigo-800/50">
                      ...
                      <NavLink
                        ...
                        className={({ isActive }) =>
                          `flex items-center p-3 rounded-xl transition-all duration-200 ${
                            isActive ? 'bg-indigo-500 shadow-lg shadow-indigo-500/30' : 'hover:bg-indigo-800/30'
                          }`
                        }
                      >
                        <item.icon className="h-6 w-6" />
                        <span className="ml-4 font-medium text-indigo-100">{item.label}</span>
                      </NavLink>
                    ))}
                  </nav>
      ...
      <aside className={`${sidebarClasses} ${isSidebarCollapsed ? collapsedSidebar : expandedSidebar} hidden md:flex`}>
        {}
        <div className="flex items-center justify-between p-4 border-b border-indigo-700/50 h-16">
          {!isSidebarCollapsed && (
            <div className="flex items-center gap-2">
              <BuildingOffice2Icon className="h-8 w-8 text-blue-300" />
              <span className="text-xl font-bold bg-gradient-to-r from-blue-300 to-indigo-300 bg-clip-text text-transparent">EstateFlow</span>
        ...
        <nav className="flex-1 px-2 py-8 space-y-3">
          {navItems.map((item) => (
            ...
            >
              ...
              {!isSidebarCollapsed && <span className="ml-4 font-medium text-indigo-100">{item.label}</span>}
            </NavLink>
          ))}
        </nav>
          ...
          <div className="p-4 border-t border-indigo-700/50">
            <div className="flex items-center gap-3">
              ...
              <div className="text-left">
                <p className="text-sm font-semibold text-indigo-100">{user.name}</p>
      ...
      <div className={`${mainContentClasses} ${isSidebarCollapsed ? collapsedMainContent : expandedMainContent}`}>
        ...
        <main className="p-4 md:p-6 overflow-y-auto">
          <Routes>
            ...
            <Route path="appointments" element={<SellerAppointments/>} />
